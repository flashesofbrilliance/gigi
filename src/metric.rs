...
// ────────────────────────────────────────────────────────────────────
// Cross-bundle divergence (KL + Jensen–Shannon)
// ────────────────────────────────────────────────────────────────────

/// Output of a divergence query between two bundles.
///
/// `kl_forward` / `kl_reverse` — directional KL on the joint of common
/// numeric fiber fields (Σ over fields).
/// `jensen_shannon` — symmetric JS via the moment-matched mixture.
...
/// KL + Jensen–Shannon divergence between two bundles.
///
/// Uses Gaussian KL computed from per-field sufficient statistics (μ, σ²)
/// cached in the bundle. Field discovery and computation are both O(1)
/// w.r.t. record count after the first access (which warms the stats cache).
///
...
pub fn kl_divergence_ref(
    ...
    for field in &common_fields {
        ...
        // KL(a‖b) for Gaussian
        let kl_ab = 0.5
            * (var_a / var_b + (mu_a - mu_b).powi(2) / var_b - 1.0 + (var_b / var_a).ln());
        let kl_ba = 0.5
            * (var_b / var_a + (mu_b - mu_a).powi(2) / var_a - 1.0 + (var_a / var_b).ln());

        // Moment-matched mixture M = ½P + ½Q
        let mu_m = (mu_a + mu_b) / 2.0;
        let var_m = (var_a + var_b) / 2.0 + (mu_a - mu_b).powi(2) / 4.0;
        let kl_am = 0.5
            * (var_a / var_m + (mu_a - mu_m).powi(2) / var_m - 1.0 + (var_m / var_a).ln());
        let kl_bm = 0.5
            * (var_b / var_m + (mu_b - mu_m).powi(2) / var_m - 1.0 + (var_m / var_b).ln());
